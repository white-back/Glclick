...
class VisionTransformer(nn.Module):
    ...
    def forward_backbone(self, x, additional_features=None,coord_feats=None,shuffle=False):
            ...
            for i in range(1, num_blocks + 1):
                if i % num_blocks_per_group:
                    if not is_patchified:
                        x = self.patchify(x)
                        #additional_features = self.patchify(additional_features)
                        #x += additional_features
                        is_patchified = True
                    else:
                        pass # do nothing
                    ...
                    x = self.unpatchify(x)
                    x += coord_feats[i//12]
                    #additional_features = self.unpatchify(additional_features)
                    # my
                    #x += additional_features
                    is_patchified = False
                x = self.blocks[i-1](x)
